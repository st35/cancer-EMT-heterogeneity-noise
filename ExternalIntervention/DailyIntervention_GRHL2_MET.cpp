...
void initialize_Signal_lognormal(population &P, int N)
{
	...
	for(int i = 0; i < N; i++)
	{
		boost::numeric::odeint::integrate(SNAIL_ZEB_miR200_miR34_GRHL2_system, P[i], 0.0, 7500.0, 0.1);
	}
}

void get_phenotypes(population &P, boost::unordered_map <int, int> &phenotype)
{
	double x1 = 57.2, x2 = 142.52, y1 = 659.498, y2 = 444.566;
	double a1 = y2 - y1, b1 = x1 - x2, c1 = x2*y1 - y2*x1;

	double u1 = 75.88, u2 = 119.56, v1 = 220.21, v2 = 75.5411;
...
void simulate_normal(population &P, double end_time, double eta, int eta_id, int sim_type, int rank, boost::array <double, 3> &GR)
{
	...
	double last_updated = 0.0, DoseT = 0.0;
	...
	std::ofstream count_file, sigfile;
	if(sim_type != -1)
	{
		...
		sigfile.open("outputfiles/RUN" + std::to_string(eta_id) + "/sig_file_" + std::to_string(sim_type) + '_' + std::to_string(eta_id) + '_' + std::to_string(rank) + ".log");
	...
	int sig_flag1 = 0, sig_flag2 = 0, sig_flag3 = 0, sig_flag4 = 0, sig_flag5 = 0;
	...
	while(t <= end_time)
	{
		...
		if(event_id < 3)
		{
			boost::numeric::odeint::integrate(SNAIL_ZEB_miR200_miR34_GRHL2_system, P[rep_id], 0.0, t - update_time[rep_id], 0.1);
			get_phenotypes(P, phenotype);
		...
		if(true)
		{
			for(int i = 0; i < P.size(); i++)
			{
				...
				{
					boost::numeric::odeint::integrate(SNAIL_ZEB_miR200_miR34_GRHL2_system, P[i], 0.0, t - update_time[i], 0.1);
				}
				update_time[i] = t;
			}
		}
		...
		if(sim_type != -1)
		{
			...
			if(t - DoseT > 24.0 && t > 24.0*10.0 && t <= 24.0*20.0)
			{
				for(int i = 0; i < P.size(); i++)
				{
					P[i][6] += DoseStep;
...
int main(int argc, char *argv[])
{
	...
	if(FACSFlag)
	{
		simulate_normal(P_mes, 24.0*365, eta, eta_id, 0, world_rank, GR);
	}
	else
	{
