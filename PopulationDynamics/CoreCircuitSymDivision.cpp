...
void add_sym_noise_normal(population &P, int rep_id, int new_id, int rep_phenotype, double eta)
{
...
void simulate_normal(population &P, double end_time, double eta, int eta_id, int sim_type, int rank, boost::array <double, 3> &GR)
{
	...
	while(t <= end_time)
	{
		...
		if(event_id < 3)
		{
			...
			add_sym_noise_normal(P, rep_id, last_index, phenotype[rep_id], eta);
