...
boost::array<double, 3> DRUG = {0.0, 0.0, 0.0};
double drugLambda = 20.0, drugN = 4.0, drug0 = 50e3;

double shiftedHill(double drugConc, double drug0, double drugN, double drugLambda)
{
	return(drugLambda + (1.0 - drugLambda)*(1.0 / (1.0 + std::pow(drugConc / drug0, drugN))));
}
...
void simulate_normal(population &P, double end_time, double eta, int eta_id, int sim_type, int rank, boost::array <double, 3> &GR)
{
	...
	for(int i = 0; i < 3; i++)
	{
		d0[i] = d0[i]*shiftedHill(DRUG[i], drug0, drugN, drugLambda);
	}
...
int main(int argc, char *argv[])
{
	...
	DRUG[0] = std::stod(argv[6]);
	DRUG[1] = std::stod(argv[7]);
	DRUG[2] = std::stod(argv[8]);

	simulate_normal(P, 24.0*32, eta, eta_id, 3, world_rank, GR);
